...
impl HummockManager {
    ...
    fn decode_checkpoint_data(
        ...
        if let Ok(envelope) = PbHummockVersionCheckpointEnvelope::decode(data.clone()) {
            if !envelope.payload.is_empty() {
                let algo = CheckpointCompressionAlgorithm::try_from(envelope.compression_algorithm)
                    .unwrap_or(CheckpointCompressionAlgorithm::CheckpointCompressionNone);
                ...
                let ckpt = PbHummockVersionCheckpoint::decode(decompressed.as_ref())
                    .map_err(|e| anyhow::anyhow!("failed to decode checkpoint payload: {}", e))?;
                tracing::info!(
                    compression = ?algo,
        ...
        PbHummockVersionCheckpoint::decode(data.clone())
            .map_err(|e| anyhow::anyhow!("failed to decode legacy checkpoint: {}", e))
    }
    ...
    fn decompress_payload(
        ...
        match algo {
            CheckpointCompressionAlgorithm::CheckpointCompressionNone => Ok(payload.to_vec()),
            ...
            CheckpointCompressionAlgorithm::CheckpointCompressionLz4 => {
                let mut decoder = lz4::Decoder::new(payload)
                    .map_err(|e| anyhow::anyhow!("lz4 decoder init failed: {}", e))?;
    ...
    fn compress_payload(
        ...
        match algo {
            CheckpointCompressionAlgorithm::CheckpointCompressionNone => Ok(data.to_vec()),
            CheckpointCompressionAlgorithm::CheckpointCompressionZstd => {
            ...
            CheckpointCompressionAlgorithm::CheckpointCompressionLz4 => {
                ...
                let mut encoder = lz4::EncoderBuilder::new()
                    ...
                    .map_err(|e| anyhow::anyhow!("lz4 encoder init failed: {}", e))?;
                std::io::Write::write_all(&mut encoder, data)
                    .map_err(|e| anyhow::anyhow!("lz4 compression write failed: {}", e))?;
                let (_writer, result) = encoder.finish();
                result.map_err(|e| anyhow::anyhow!("lz4 compression finish failed: {}", e))?;
                Ok(compressed)
            }
        }
    }
